...
void update(ll *st,int ss, int se, int s_idx, int i,int dif){
    if(i < ss || se < i) return;
    st[s_idx] += dif;
    int mid = getMid(ss,se);
    if(ss!=se){
        update(st, ss, mid, 2*s_idx+1, i , dif);
        update(st, mid+1, se, 2*s_idx+2, i , dif);
    }
}
int main(){
    int n,q;
    ...
    while(q--){
        ...
        if(o%2){
            ll dif = b - arr[a-1];
            arr[a-1] = b;
            update(segment_tree,0,n-1,0,a-1,dif);
        }else cout<<getSum(segment_tree,0,n-1,a-1,b-1,0)<<endl;
    }
